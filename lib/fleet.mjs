...
// relative to that manifest first, then to the plugin.
function loadFleet(fleetName) {
  ...
  const expand = (placement) => {
    if (!placement.stack) {
      ...
      return [{ ...placement, sourcePath, granularity: "per-resource" }];
    }
    ...
    const stackDir = dirname(stackPath);
    return (stack.spec?.components ?? []).map((comp) => {
      const source = comp.render ?? comp.authored ?? null;
      const sourcePath = source ? [join(stackDir, source), join(pluginRoot, source)].find((candidate) => existsSync(candidate)) : null;
      ...
      return {
        ...
        authored: source,
        sourcePath,
        granularity: comp.render ? "per-file" : "per-resource",
...
const spaceExists = (space) => {
  try { cub(["space", "get", space, "-o", "name"]); return true; }
    ...
    // The CLI's explicit missing-Space response is the only absence evidence.
    // Authentication, network and permission failures must never trigger creation.
    const lines = String(error.stderr || error.stdout || error.message).split("\n").map(line => line.trim().replace(/^Failed: /, ""));
    if (lines.includes(`space ${space} not found`)) return false;
    throw error;
  }
};
    ...
    for (const placement of fleet.placements) {
      if (!spaceExists(`${placement.app}-base`)) {
        ...
        if (placement.bundle) {
          ...
          cub(["variant", "upload", "--component", placement.app, "--variant", "base", "--granularity", placement.granularity ?? "per-resource", "--owner", owner, placement.sourcePath ?? join(pluginRoot, placement.authored)]);
        }
