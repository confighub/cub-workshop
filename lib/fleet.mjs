...
function loadFleet(fleetName) {
  ...
  const placements = (fleet.spec?.placements ?? []).map((placement) => ({
    ...placement,
...
if (verb === "list") {
  ...
  console.log(`\nFleet ${fleet.name}: ${fleet.clusterNames.length} clusters, ${fleet.placements.length} components\n`);
  ...
  for (const placement of fleet.placements) {
    ...
    console.log(`  ${placement.app}: placed and released on ${placement.clusters.length} cluster(s)`);
  }
  ...
  for (const op of fleet.demoAging) {
    if (op.kind === "pending" || op.kind === "advance-base") {
      ...
      const [key, value] = op.annotation.split("=");
      ...
      cub(["trigger", "create", "require-approval", "Mutation", "Kubernetes/YAML", "vet-approvedby", "1", "--space", op.space]);
      ...
      cub(["function", "do", "--space", `${op.component}-base`, "--where", "Slug LIKE '%deployment%'",
  ...
  for (const placement of fleet.placements) order.push(`${placement.app}-base`);
  let deleted = 0;
  for (const space of order) { try { cub(["space", "delete", space, "--recursive"]); deleted += 1; } catch { /* absent */ } }
  console.log(`fleet ${fleet.name} teardown complete: ${deleted} space(s) removed`);
} else {
