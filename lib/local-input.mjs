...
export function loadInput(input, directory, noun) {
  ...
  if (!docs.length || docs.some(doc => typeof doc !== 'object' || Array.isArray(doc) || [doc.apiVersion, doc.kind, doc.metadata?.name].some(value => typeof value !== 'string' || !value.trim()))) {
    // A Helm values file is the commonest thing handed to check by mistake: it is YAML,
    // it is about one chart, and it has no apiVersion or kind. Say where it belongs.
    const looksLikeValues = docs.length === 1 && docs[0] !== null && typeof docs[0] === 'object' && !Array.isArray(docs[0])
      && !docs[0].apiVersion && !docs[0].kind;
    throw new Error(`${noun} input must contain named Kubernetes objects with apiVersion, kind and metadata.name${looksLikeValues ? '; for a Helm values file, use cub config values <chart> --values ' + (local ? path : input) : ''}`);
  }
