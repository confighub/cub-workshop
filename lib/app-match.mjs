...
import { parseBoundedDocuments, readBoundedBytes } from "./local-input.mjs";
...
function documents(bytes, label) {
  let parsed;
  try { parsed = parseBoundedDocuments(bytes, label); }
...
export function runMatch(args) {
  ...
  const result = matchWorkload(readBoundedBytes(input), readBoundedBytes(target));
