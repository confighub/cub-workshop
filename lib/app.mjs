...
//
//   cub app list
//   cub app check <name | local.yaml>            render it for free; find out if it needs a platform
...
if (verb === "list") {
  ...
  if (!name) {
    console.error("usage: cub app check <name | local.yaml>");
    process.exit(2);
  }
  ...
  const docs = splitRawDocs(app.bytes.toString("utf8"));
  ...
  if (!RUN) {
    console.log(`  Dry run. Add --run to upload, then \`cub variant approve ${space} --all\` releases the gated app.\n`);
    process.exit(0);
  }
  ...
  try {
    ...
    console.log(`  The review: cub variant approve ${space} --all`);
...
Usage:
  cub app list
  cub app check <name | local.yaml> [--out oci://<repo>[:tag]]
