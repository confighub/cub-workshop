...
// The platform services an app can depend on live in needs.mjs, shared with certify.
...
if (verb === "list") {
  ...
  if (OUT && OUT.startsWith("oci://")) {
    const content = readFileSync(app.path);
    const files = [{ path: `${app.name}.yaml`, content }];
    const services = [...new Set(deps.map((dep) => dep.service))];
    const checks = [
      ["PASS", `${app.objects.length} objects`],
      [services.length ? "NEEDS" : "PASS", services.length ? `needs a platform for ${services.join(", ")}` : "standalone"],
      ["PASS", `namespaces that must already exist: ${nsPrereqs.join(", ") || "none"}`],
    ];
    const receipt = buildReceipt({ name: app.name, source: { kind: "authored", name: app.name, origin: "cub-workshop apps/" }, files, checks });
...
Usage:
  cub app list
  cub app check <name> [--out oci://<repo>[:tag]]
