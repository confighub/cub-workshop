...
export function elsewhere(defaults, path, limit = 3) {
  ...
  const walk = (node, prefix) => {
    if (!isMap(node) || Object.keys(node).length === 0) { if (prefix.length) declared.push(prefix); return; }
  ...
  for (let take = path.length - 1; take >= 1; take -= 1) {
    ...
    if (hits.length) return hits.sort((left, right) => left.length - right.length).slice(0, limit).map(show);
  }
  return [];
}
...
export function diagnose({ values, defaults, render, limit = MAX_RENDERS }) {
  ...
  const moving = unstableFields(baseline, render(values));
  const results = [];
  let renders = 0;
...
export function runValues(args) {
  ...
  if (!chart || !valuesPath) {
    console.error("usage: cub config values <chart> --values my-values.yaml [--version X] [--repo URL] [--namespace ns] [--release name] [--json] [--exit-code]");
    return 2;
  }
...
function print(report, chart) {
  ...
  if (report.unstableFields) console.log(`  ${report.unstableFields} generated field${report.unstableFields === 1 ? "" : "s"} (passwords, checksums) ${report.unstableFields === 1 ? "was" : "were"} left out of the comparison.`);
