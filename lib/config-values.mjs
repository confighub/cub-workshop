...
import { parseYaml, readYamlFile, sha256, toYaml } from "./common.mjs";
...
export function runValues(args) {
  ...
  const takesValue = new Set(["--values", "-f", "--version", "--repo", "--namespace", "--release", "--out"]);
  const chart = args.find((arg, index) => !arg.startsWith("-") && !takesValue.has(args[index - 1]));
  const valuesPath = flag("--values") ?? flag("-f");
  const usage = "usage: cub config values <chart> --values my-values.yaml [--version X] [--repo URL] [--namespace ns] [--release name] [--json] [--out result.json] [--exit-code]";
  ...
  // Reserve the result before pulling or rendering the chart. A second run must
  // choose a new path rather than overwrite the evidence from the first one.
  let outFile = out ? openSync(out, "wx", 0o600) : null;
  ...
  try {
    ...
    if (outFile !== null) { writeFileSync(outFile, serialized); closeSync(outFile); outFile = null; wroteResult = true; }
    ...
    if (out && !wroteResult) rmSync(out, { force: true });
