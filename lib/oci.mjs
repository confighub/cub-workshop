// A bundle with its receipt, as an OCI artifact: build it reproducibly, push it,
...
export function verifyBundle(reference, { key = null } = {}) {
  ...
  const findings = [];
  let ok = true;
  // A file is matched by its exact path, then by basename anywhere in the
  // layer, then by content hash under any name. The bytes are what the
  // receipt certifies; a name that drifted is reported, not refused.
  const byPath = new Map(pulled.files.map((file) => [file.path, file.content]));
  const byHash = new Map(pulled.files.map((file) => [sha256(file.content), file.path]));
  let renamed = 0;
  for (const file of receipt.spec?.bundle?.files ?? []) {
    const base = file.path.split("/").pop();
    const content = byPath.get(file.path) ?? byPath.get(base) ?? pulled.files.find((entry) => entry.path.endsWith(`/${base}`))?.content;
    if (content) {
      if (sha256(content) !== file.sha256) { ok = false; findings.push(["FAIL", `${file.path} does not match its recorded hash`]); }
      continue;
    }
    const underOtherName = byHash.get(file.sha256);
    if (underOtherName) { renamed += 1; continue; }
    ok = false;
    findings.push(["FAIL", `${file.path} is listed in the receipt but missing from the bundle`]);
  }
  if (renamed) findings.push(["NOTE", `${renamed} file(s) present with the recorded bytes under a different name than the receipt lists`]);
