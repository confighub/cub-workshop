...
function bundleSourcePath(listing, id, fallback) {
  const flattened = listing.flattened;
  if (!flattened || !Object.hasOwn(flattened, "bundleSourcePath")) return fallback;
  const candidate = flattened.bundleSourcePath;
  if (typeof candidate !== "string" || !candidate || candidate.includes("\\") || isAbsolute(candidate) || /^[A-Za-z]:/.test(candidate)) {
    fail(`${id}.flattened.bundleSourcePath must be a non-empty repository-relative path using forward slashes`, 1, "source_invalid");
  }
  const segments = candidate.split("/");
  if (segments.some((segment) => !segment || segment === "." || segment === "..")) {
    fail(`${id}.flattened.bundleSourcePath must not contain empty, ".", or ".." path segments`, 1, "source_invalid");
  }
  return candidate;
}
...
async function routeBundle(listing, id, listingRead) {
  ...
  const expectedSourcePath = bundleSourcePath(listing, id, `packages/${listing.identity?.name}/${listing.identity?.version}/bases/${listing.identity?.base}/upstream.yaml`);
