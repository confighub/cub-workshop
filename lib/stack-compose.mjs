...
async function loadListings(options) {
  ...
  for (const id of options.entries) {
    ...
    if (isHttp(listingRef) && !isHttps(listingRef)) fail(`${id}.url must use https`, 1);
    ...
    if (isHttp(retained.url) && !isHttps(retained.url)) fail(`${id}.flattened.retainedObjects.url must use https`, 1);
    if (listingRead.remote && !isHttps(retained.url)) fail(`${id}.flattened.retainedObjects.url must be https when the listing is remote`, 1);
    if (!listingRead.remote && isAbsolute(retained.path)) fail(`${id}.flattened.retainedObjects.path must be relative to its listing`, 1);
    if (retained.path.split(/[\\/]/).includes("..")) fail(`${id}.flattened.retainedObjects.path must not contain path traversal`, 1);
    ...
    if (!Number.isInteger(listing.flattened.objectCount) || listing.flattened.objectCount <= 0) fail(`listing ${id} has an invalid flattened.objectCount`, 1);
    if (objects.some((object) => !object || typeof object !== "object" || Array.isArray(object)
      ...
      fail(`retained objects for ${id} contain an empty, unnamed, or malformed Kubernetes document`, 1);
    }
