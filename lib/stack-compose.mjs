...
async function routeBundle(listing, id, listingRead) {
  ...
  if (receipt?.kind !== "CertifiedBundleReceipt" || bundleSpec?.manifestDigest !== manifest || bundleSpec?.reference !== expectedReceiptReference || bundleSpec?.objectCount !== listing.flattened?.objectCount || configuration.length !== 1 || source?.path !== expectedSourcePath || source?.sha256 !== objects.slice(7) || source?.sha256 !== listing.flattened?.retainedObjects?.sha256?.slice(7) || !validRoutes || receipt.spec?.verdict?.lane !== "flatten-with-routes" || receipt.spec?.verdict?.status !== "certified" || !exactChart) {
