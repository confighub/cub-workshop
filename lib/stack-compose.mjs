...
async function loadListings(options) {
  ...
  for (const id of options.entries) {
    ...
    if (!ROLES_VERDICTS.has(verdict)) fail(`listing ${id} is ${verdict ?? "missing a flattening verdict"}; compose accepts only safe-to-flatten or born-flat`, 1, verdict === "flatten-with-routes" ? "lifecycle_route_required" : "source_invalid");
