...
if (verb === "list") {
  ...
  if (!name) fail("usage: cub stack from-kubara <kubara-work-dir> [--cluster <name>] [--out <dir>]");
  ...
  names.forEach((comp, index) => {
    ...
    writeFileSync(join(outDir, "renders", `${comp}.yaml`), rendered);
    console.log(`  ${comp}: ${objects.length} objects from Kubara's chart and ${valuesFiles.length} values file(s), namespace ${namespace}`);
    components.push({ name: `${CLUSTER}-${comp}`, plane: "mgmt", order: index, render: `renders/${comp}.yaml` });
  });
  const manifest = {
...
Usage:
  ...
  cub stack from-kubara <kubara-work-dir> [--cluster <name>] [--out <dir>]   # Kubara's generated platform as a stack, rendered with its own values
