...
if (verb === "list") {
  ...
  if (JSON_OUTPUT) {
    ...
    const content = objects.map((obj) => toYaml(obj)).join("---\n");
    // Reuse the published receipt's hashing and check fields. This result is
    // not a published bundle or a claim that the target has been qualified.
    const receipt = buildReceipt({ name: stack.name, source: { kind: "stack", name: stack.name },
      files: [{ path: `${stack.name}.yaml`, content }], checks: result.findings });
