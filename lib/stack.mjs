...
//
//   cub stack list
//   cub stack check <name>       read the composition; exits non-zero on a conflict
//   cub stack sandbox <name>     check, then render the composition for free
...
import { canonicalMap, cub, fail, identity, parseDocs, pluginRoot, readYamlFile, resolveBundle, resolveBundleDetails, sha256, toYaml } from "./common.mjs";
...
if (JSON_OUTPUT && (!CHECKS.has(verb) || args.length !== 2 || name.startsWith("--"))) {
  fail("usage: cub stack check <name | manifest | OCI reference> --json (no other flags)");
}
...
function restoreWorkspaceEvidence(path, stack, components) {
  ...
  if (saved?.kind !== "StackCertificationResult" || saved.name !== (stack.metadata?.name ?? "") || !saved.lifecycleCompanions) return;
  ...
  if (!manifestHash || sha256(readFileSync(path)) !== manifestHash) fail("workspace lifecycle evidence is stale because stack.yaml changed; preserve the original workspace evidence or create a new stack without it");
...
function loadStack(stackName, { preserveWorkspaceEvidence = false } = {}) {
  if (String(stackName).startsWith("oci://")) {
    const loaded = loadIndexStack(stackName);
    const components = loaded.stack.spec.components.map((comp) => {
...
function certify(stack) {
  ...
  return { certified: hardFailures === 0, findings, objectCount, prerequisites };
}
...
if (verb === "list") {
  ...
  if (verb === "sandbox") {
    if (result.certified) {
      ...
      if (!WORKSPACE) console.log(`\n  Ready. \`cub stack upload ${name.startsWith("oci://") ? stack.name : name} --run\` builds the base Spaces and links in ConfigHub.\n`);
    } else {
  ...
  const record = {
    ...
    spec: { manifest: readFileSync(stack.path, "utf8"), verdict: result.certified ? "CHECKED" : "REFUSED", checks: result.findings.filter(([mark]) => mark.trim()).map(([mark, text]) => ({ result: mark, text })), components: entries },
  };
  ...
  if (!name) fail("usage: cub stack upload <name | manifest> [--space-prefix <prefix>] [--run]");
  const stack = loadStack(name);
  const result = certify(stack);
  printHeader(stack);
  printCertify(result);
  ...
  const spacePattern = PREFIX ? ["--space-pattern", `template:${PREFIX}-{{.Labels.Component}}`] : [];
  const steps = stack.components.map((comp) => comp.bundle
    ? ["variant", "upload", "--component", comp.name, "--variant", "base", "--owner", stack.name, ...spacePattern, comp.bundle]
    : ["variant", "upload", "--component", comp.name, "--variant", "base", "--owner", stack.name, ...spacePattern, comp.sourcePath ?? join(pluginRoot, comp.render ?? comp.authored)]);
...
Usage:
  ...
  cub stack check <name | path/to/manifest.yaml | oci://<repo>@sha256:<index digest>> [--json]   (certify is the old name for this)
