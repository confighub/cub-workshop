...
import { copyIntoRepo, parseReference, publishBundle, publishIndex, verifyBundle } from "./oci.mjs";
...
if (verb === "list") {
  ...
  for (const comp of stack.components) {
    if (comp.bundle) {
      const digest = copyIntoRepo(comp.bundle, target.repo, { plain: target.plain });
...
Usage:
  ...
  cub stack sandbox <name | path/to/manifest.yaml> [--out rendered.yaml | --out oci://<repo>[:tag]]
  cub stack publish <name> --out oci://<repo>[:tag]      # the index of images, manifest and verdict attached
