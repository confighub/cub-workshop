...
if (JSON_OUTPUT && (verb !== "certify" || args.length !== 2 || name.startsWith("--"))) {
  fail("usage: cub stack certify <name | manifest | OCI reference> --json (no other flags)");
}
  ...
  if (verb === "sandbox") {
    if (result.certified) {
      ...
      if (WORKSPACE) {
        ...
        console.log(`  Resume: cub stack certify ${quoted} --json`);
...
Usage:
  cub stack list
  cub stack certify <name | path/to/manifest.yaml | oci://<repo>@sha256:<index digest>> [--json]
