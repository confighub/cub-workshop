...
import { resourceRequirementsFindings } from "./resource-requirements.mjs";
...
function parseCheckArgs(values) {
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    if (arg === "--images") {
      if (images) throw new Error("--images may be specified only once");
      images = true;
    } else if (arg === "--exit-code") {
      if (exitCode) throw new Error("--exit-code may be specified only once");
      exitCode = true;
    } else if (arg === "--out" || arg === "--sign") {
      const value = values[++index];
      if (!value || value.startsWith("--")) throw new Error(`${arg} requires a value`);
      if (arg === "--out") {
        if (out) throw new Error("--out may be specified only once");
        out = value;
      } else {
        if (sign) throw new Error("--sign may be specified only once");
        sign = value;
      }
  ...
  if (!name) throw new Error("a config name or local.yaml is required");
  if (exitCode && !images) throw new Error("--exit-code requires --images");
  return { name, images, exitCode, out, sign };
}
...
if (verb === "list") {
  ...
  let options;
  try { options = parseCheckArgs(args.slice(1)); }
  catch (error) {
    console.error(`usage: cub config check <name | local.yaml> [--images] [--exit-code] [--out oci://<repo>[:tag] [--sign cosign.key] | --out file.yaml]\n${error.message}`);
    process.exit(2);
  }
  const { images: IMAGES, exitCode: EXIT_CODE, out: OUT, sign: SIGN } = options;
  const config = loadConfig(options.name);
  const invalidResources = resourceRequirementsFindings(config.objects);
  if (invalidResources.length) {
    console.log(`\nConfig: ${config.name}\n`);
    for (const finding of invalidResources) {
    ...
    console.log("\n  Fix the fields above before using this configuration. This is a limited field check, not full Kubernetes schema or admission validation.\n");
    process.exit(1);
  }
  ...
  let pulls = [];
  if (IMAGES) {
    pulls = canPull(facts.images);
    const broken = pulls.filter((entry) => entry.result !== "pulls");
    const missing = broken.filter((entry) => entry.result === "NOT FOUND");
    const incomplete = broken.filter((entry) => entry.result !== "NOT FOUND");
    const loose = pulls.filter((entry) => entry.result === "pulls" && !entry.pinned && entry.digest);
    console.log(`  ${missing.length ? "[FAIL]" : incomplete.length ? "[INCOMPLETE]" : "[PASS]"} images that pull anonymously: ${pulls.length - broken.length} of ${pulls.length}`);
    ...
    if (EXIT_CODE && missing.length) {
      console.log("\n  [FAIL] one or more images are confirmed missing; no output was written.\n");
      process.exit(1);
    }
    if (EXIT_CODE && incomplete.length) {
      console.log("\n  [INCOMPLETE] anonymous registry checks could not verify every image; no output was written.\n");
      process.exit(2);
    }
  }
  ...
  if (OUT && OUT.startsWith("oci://")) {
    ...
    const checks = [
      ...
      ...(IMAGES ? [[pulls.every((entry) => entry.result === "pulls") ? "PASS" : "NOTE", `images that pull anonymously, checked at ${new Date().toISOString().slice(0, 10)}`]] : []),
