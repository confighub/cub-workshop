...
function parseListArgs(values) {
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    if (arg === "--json") {
      ...
      if (catalogIndex !== DEFAULT_CATALOG_INDEX) throw new Error("--catalog-index may be specified only once");
  ...
  if (!role && (json || catalogIndex !== DEFAULT_CATALOG_INDEX)) throw new Error("--json and --catalog-index require --role");
...
async function readCatalogIndex(source) {
  ...
  if (/^https:\/\//i.test(source)) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`catalog index request failed with HTTP ${response.status}`);
      text = await response.text();
    } catch (error) {
      if (error.name === "AbortError") throw new Error("catalog index request timed out after 10 seconds");
      throw new Error(`could not read catalog index: ${error.message}`);
    } finally {
      clearTimeout(timer);
    }
  } else if (/^https?:\/\//i.test(source)) {
    throw new Error("catalog index must be a local file or an https URL");
  } else {
    try { text = readFileSync(source, "utf8"); }
    catch (error) { throw new Error(`could not read catalog index file: ${error.message}`); }
  }
  let index;
  try { index = JSON.parse(text); }
  catch (error) { throw new Error(`catalog index is not valid JSON: ${error.message}`); }
  if (!index || typeof index !== "object" || Array.isArray(index) || !Array.isArray(index.listings)) {
    throw new Error("catalog index is malformed: expected an object with a listings array");
  }
  for (const [position, listing] of index.listings.entries()) {
    if (!listing || typeof listing !== "object" || Array.isArray(listing)) throw new Error(`catalog index listing ${position + 1} is malformed`);
    for (const field of ["id", "url", "version", "base"]) {
      if (typeof listing[field] !== "string" || listing[field].length === 0) throw new Error(`catalog index listing ${position + 1} has an invalid ${field}`);
    }
    ...
    if (!discovery || typeof discovery !== "object" || Array.isArray(discovery) || !["classified", "not-classified"].includes(discovery.status)) {
      throw new Error(`catalog index listing ${listing.id} has an unknown discovery status`);
    }
    if (!Array.isArray(discovery.roles)) throw new Error(`catalog index listing ${listing.id} has malformed discovery roles`);
    for (const candidate of discovery.roles) {
      if (!candidate || typeof candidate !== "object" || !ROLES.has(candidate.role) || !COMPONENT_TYPES.has(candidate.componentType)) {
        throw new Error(`catalog index listing ${listing.id} has a malformed discovery role`);
      }
    }
  }
  return index;
...
if (verb === "list") {
  try {
    ...
    throw new Error(`usage: cub config list [--role ROLE] [--json] [--catalog-index FILE_OR_HTTPS_URL]\n${error.message}`);
  }
} else if (verb === "check") {
  if (!name) {
...
Usage:
  ...
  cub config values <chart> --values my-values.yaml [--version X] [--repo URL] [--json] [--exit-code]
  cub config list
