...
//
//   cub config list
//   cub config check <name | local.yaml>   render it for free and see what it installs
//
...
function analyze(config) {
  ...
  for (const obj of config.objects) {
    for (const pod of podSpecs(obj)) {
      for (const container of [...(pod.initContainers ?? []), ...(pod.containers ?? []), ...(pod.ephemeralContainers ?? [])]) {
        ...
        if (!image || image.includes("@sha256:")) continue;
  ...
  return { kinds, nsPrereqs, crds, hooks, jobs, webhooksNeedingCa, floating: [...floating].sort() };
}
...
if (verb === "list") {
  ...
  console.log(`\ncub config check <name | local.yaml>   # render and check, free\n`);
} else if (verb === "check") {
  if (!name) {
    console.error("usage: cub config check <name | local.yaml>");
    process.exit(2);
  }
...
Usage:
  ...
  cub config list
  cub config check <name | local.yaml> [--out oci://<repo>[:tag] [--sign cosign.key] | --out file.yaml]
