...
function canPull(images) {
  return images.map((image) => {
    ...
    try {
      execFileSync("oras", ["manifest", "fetch", "--descriptor", reference], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], timeout: 60000, env: { ...process.env, DOCKER_CONFIG: "/nonexistent-so-the-pull-is-anonymous" } });
      return { image, result: "pulls" };
    } catch (error) {
