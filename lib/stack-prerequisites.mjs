...
export function stackPrerequisites(stack) {
  ...
  const clusterKinds = new Set(['Namespace', 'ClusterIssuer', 'ClusterSecretStore', 'IngressClass']);
  const namespace = o => o.metadata?.namespace ?? null;
  const supplied = new Set(objects.map(({ object: o }) => key({ group: groupOf(o), kind: o.kind,
    namespace: clusterKinds.has(o.kind) ? null : namespace(o), name: o.metadata?.name })));
  const requirements = new Map();
  function add(component, o, field, group, kind, ns, name) {
    if (!name) return;
    const ref = { group, kind, namespace: ns, name };
    const id = key(ref);
    // The caller may choose an apply namespace; an omitted namespace is not
    // evidence of 'default' and cannot establish a namespaced reference match.
    const present = (clusterKinds.has(kind) || ns !== null) && supplied.has(id);
    if (!requirements.has(id)) requirements.set(id, { ...ref, status: present ? 'bundled' : 'unknown', consumers: [],
      remedy: present ? 'Verify readiness on the selected target before delivery.' : `Include ${kind}/${name}${ns ? ` in namespace ${ns}` : clusterKinds.has(kind) ? '' : ' in the target namespace (not specified)'} in the stack, or verify it exists on the selected target before delivery.` });
