...
test('malformed quantities, inconsistent requests and duplicate nodes refuse', () => {
  ...
  assert.throws(() => matchWorkload(model, mutate(target, (d) => { d.items.push(structuredClone(d.items[0])); })), /duplicate Node/);
});
...
test('CLI rejects malformed UTF-8, secrets and oversized or nonregular inputs safely', () => {
  const dir = mkdtempSync(join(tmpdir(), 'workshop-match-bounds-'));
  const run = (args) => spawnSync(process.execPath, [join(root, 'bin/cub-app'), 'match', ...args], { cwd: dir, encoding: 'utf8', timeout: 2000 });
  try {
    writeFileSync(join(dir, 'model.yaml'), Buffer.concat([model, Buffer.from('\nprivate-token: PRIVATE_MATCH_SECRET\n'), Buffer.from([0xff])]));
    writeFileSync(join(dir, 'nodes.yaml'), target);
    const malformed = run(['model.yaml', '--target', 'nodes.yaml']);
    assert.notEqual(malformed.status, 0);
    assert.match(malformed.stderr, /UTF-8/);
    assert.doesNotMatch(malformed.stderr, /PRIVATE_MATCH_SECRET|YAMLException|line|column/);

    writeFileSync(join(dir, 'model.yaml'), Buffer.alloc(10 * 1024 * 1024 + 1, 0x20));
    const oversized = run(['model.yaml', '--target', 'nodes.yaml']);
    assert.notEqual(oversized.status, 0);
    assert.match(oversized.stderr, /byte limit/);

    const fifo = join(dir, 'nodes.yaml');
    writeFileSync(join(dir, 'model.yaml'), model);
    rmSync(fifo);
    execFileSync('mkfifo', [fifo]);
    const nonregular = run(['model.yaml', '--target', 'nodes.yaml']);
    assert.notEqual(nonregular.status, 0);
    assert.match(nonregular.stderr, /regular file/);
